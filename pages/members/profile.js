...
export default function Profile() {
  ...
  const { data, user, loading, error } = useMemberData();
  const profile = data?.profile;
  const saved = router.query.saved === '1';

  useEffect(() => {
    if (!loading && profile && !profile.onboarding_completed) router.replace('/onboarding');
  }, [loading, profile, router]);

  if (error) {
    return <Layout><SEO title="My profile | Unboxed Together" description="Manage your Unboxed Together profile and onboarding answers." path="/members/profile" /><div className="member-page"><p className="onboarding-error" role="alert">{error}</p></div></Layout>;
  }
  if (loading || !profile || !profile.onboarding_completed) {
    return <Layout><SEO title="My profile | Unboxed Together" description="Manage your Unboxed Together profile and onboarding answers." path="/members/profile" /><div className="member-page"><p className="auth-loading">Loading your profile…</p></div></Layout>;
  }

  return <ProfileContent profile={profile} email={user?.email || ''} saved={saved} />;
}

function ProfileContent({ profile, email, saved }) {
